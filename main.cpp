...
void DrawTriangle(GLfloat x1, GLfloat y1, GLfloat x2, GLfloat y2, GLfloat x3, GLfloat y3, float depth)
{
    glBegin(GL_TRIANGLES);
...
public:
    bool OnUserCreate()
    {
        string defPath = "C:\\Users\\Dmitry\\Documents\\GitHub\\KartonGameEngine\\KartonGameEngine\\resources\\";
    ...
    bool OnUserUpdate(float fElapsedTime)
    {
        ...
        for (auto tri : textureMesh.tris)
        {
            ...
            if (VectorDotProduct(normal, vCameraRay) < 0.0f)
            {
                ...
                triTransformed.triangleColor = dp;
